...
def _probe_json(path: Path) -> dict:
    ...
    try:
        ...
        raise M4BError(f"ffprobe returned invalid JSON for {path.name}.") from exc
...
def get_audio_duration(path: Path) -> float:
    ...
    try:
        ...
        raise M4BError(f"Could not determine the duration of {path.name}.") from exc

    if duration <= 0:
        raise M4BError(f"Invalid audio duration for {path.name}: {duration}")
...
def _write_concat_file(
    ...
    for chapter in chapters:
        ...
        if not path.is_file():
            raise M4BError(f"Chapter audio does not exist: {path}")
...
def verify_m4b(
    *,
    ...
    if not output_path.is_file():
        raise M4BError(f"The M4B file was not created: {output_path}")

    if output_path.stat().st_size <= 0:
        raise M4BError(f"The M4B file is empty: {output_path}")
    ...
    if not audio_streams:
        raise M4BError("M4B verification failed: no audio stream was found.")

    format_info = probe.get("format") or {}

    tags = {str(key): str(value) for key, value in (format_info.get("tags") or {}).items()}
...
def create_m4b(
    *,
    ...
    if not chapter_list:
        raise M4BError("Cannot create an M4B without any chapters.")

    if not title.strip():
        raise M4BError("Cannot create an M4B without a title.")

    if not author.strip():
        raise M4BError("Cannot create an M4B without an author.")

    if bitrate_kbps <= 0:
        raise M4BError("M4B bitrate must be greater than zero.")
    ...
    if cover_path is not None:
        ...
        if candidate.is_file():
            ...
            raise M4BError(f"Cover image does not exist: {candidate}")

    for chapter in chapter_list:
        if not chapter.title.strip():
            raise M4BError("Every M4B chapter must have a title.")

        if not Path(chapter.audio_path).is_file():
            raise M4BError(f"Chapter audio does not exist: {chapter.audio_path}")

    durations = [get_audio_duration(Path(chapter.audio_path)) for chapter in chapter_list]
    ...
    with tempfile.TemporaryDirectory(prefix="audiobook-maker-m4b-") as temp_dir_name:
