...
def extract_pdf_text(source_path: Path, output_text_path: Path) -> str:
    run_command(
        ["pdftotext", "-layout", str(source_path), str(output_text_path)],
        f"Extracting text from {source_path.name}",
    )
...
def epub_heading_is_weak(candidate: str) -> bool:
    ...
    if re.fullmatch(r"[IVXLCDM]+", candidate, re.I):
        return True
    if re.fullmatch(
        r"(one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve)", candidate, re.I
    ):
        return True
    return False
...
def choose_best_epub_heading(candidates: list[str]) -> Optional[str]:
    ...
    if (
        len(cleaned) >= 2
        and epub_heading_is_weak(cleaned[0])
        and not epub_heading_is_weak(cleaned[1])
    ):
...
def epub_item_text(item: Any) -> tuple[str, Optional[str]]:
    ...
    containers = list(soup.find_all(["ol", "ul", "table", "section", "div"]))
    ...
    heading = choose_best_epub_heading(heading_candidates)
...
def epub_is_nav_or_toc(item: Any, text: str) -> bool:
    ...
    id_value = str(getattr(item, "id", "")).lower()
    ...
    if "table of contents" in sample and word_total < 500:
        return True
...
def extract_epub_text(source_path: Path, output_text_path: Path) -> ExtractedSource:
    ...
    if structured_mode:
        ...
        for document in documents:
            ...
            if heading and heading.lower() not in document["text"][:300].lower():
                parts.append(heading)
...
def _find_pdf_cover_art(
    ...
    with tempfile.TemporaryDirectory(prefix="audiobook_pdf_cover_") as temp_directory:
        output_base = Path(temp_directory) / "cover"
        ...
        if len(data) < 5000 or not data.startswith(b"\xff\xd8\xff"):
            return None
    ...
    try:
        with zipfile.ZipFile(
            ...
            for element in document_root.iter():
                ...
                relationship_id = element.attrib.get("{" + REL_NS + "}embed")

                if relationship_id:
                    chosen_relationship = relationship_id
            ...
            target = relationship_targets.get(chosen_relationship)
            ...
            if not data or not valid_image(
                mime,
                data,
            ):
                return None
...
def _cover_with_fallback(
    source_path: Path,
    native_cover: Optional[tuple[str, bytes]],
    ...
    companion = _companion_cover_art(source_path)
...
def extract_source_text(
    ...
    output_text_path = extracted_text_dir / f"{safe_filename(book_title)}.txt"
    ...
    if suffix == ".pdf":
        ...
        native_cover = _find_pdf_cover_art(source_path)

        cover_art, cover_source = _cover_with_fallback(
            source_path,
            native_cover,
            "PDF page 1",
        )
    ...
    if suffix == ".docx":
        ...
        native_cover = _find_docx_cover_art(source_path)
    ...
    raise RuntimeError("Unsupported file type: " + source_path.suffix)
