...
def process_source(
    ...
    report_path = paths.reports_dir / f"{book_title} report.txt"
    ...
    report = [
        ...
        (f"Cover art available: {'yes' if extracted.cover_art else 'no'}"),
        "",
    ]
    ...
    if extracted.details:
        ...
        report.extend(f"- {detail}" for detail in extracted.details)
    ...
    with (
        ...
        for index, section in enumerate(
            ...
            if section.kind == "intro":
                ...
                base_heading = clean_section_heading_title(section.heading)
            ...
            text_filename = f"{book_title} - {display_number} - {clean_heading}.txt"

            mp3_filename = f"{book_title} - {display_number} - {clean_heading}.mp3"

            text_file = book_chapter_text_dir / text_filename
            ...
            if mp3_output_dir is None:
                ...
                existing_mp3_file = mp3_output_dir / mp3_filename
            ...
            can_skip_existing = (
                existing_mp3_file is not None and existing_mp3_file.exists() and not force
            )
            ...
            duration = get_audio_duration(working_mp3)
        ...
        if output_format in {"m4b", "both"}:
            ...
            m4b_path = m4b_output_dir / f"{book_title}.m4b"
    ...
    report.append(f"Actual audio length: {format_duration(actual_seconds)}")
    ...
    if m4b_result is not None:
        say(f"M4B created: {m4b_result.output_path.name}.")

        say(f"M4B chapters: {m4b_result.chapter_count}.")

    say(f"Actual audio length: {format_duration(actual_seconds)}.")

    say(f"Report saved: {report_path.name}")
