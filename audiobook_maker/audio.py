...
from .common import (
    APIC, COMM, ID3, MP3, SAMPLE_RATE, Settings, TALB, TCON, TDRC,
    TIT2, TPE1, TPE2, TRCK, run_command,
)
...
def get_audio_duration(mp3_path: Path) -> float:
    ...
    if duration <= 0:
        raise RuntimeError(f"Could not read audio duration for {mp3_path.name}: duration was not positive.")
    return duration


def create_audio_from_text(text_file: Path, mp3_file: Path, temp_dir: Path, settings: Settings) -> None:
    aiff_file = temp_dir / (mp3_file.stem + ".aiff")
    run_command(["say", "-v", settings.voice, "-r", str(settings.rate), "-o", str(aiff_file), "-f", str(text_file)], f"Creating speech audio: {mp3_file.name}")
    run_command(
        ["ffmpeg", "-y", "-i", str(aiff_file), "-ar", str(SAMPLE_RATE), "-codec:a", "libmp3lame", "-b:a", f"{settings.bitrate}k", str(mp3_file)],
        f"Converting to MP3: {mp3_file.name}",
    )


def tag_mp3(mp3_file: Path, book_title: str, author: str, track_title: str, track_number: int, total_tracks: int, settings: Settings, cover_art: Optional[tuple[str, bytes]] = None) -> None:
    if ID3 is None:
        raise RuntimeError("ID3 tagging needs mutagen. Install it with: python3 -m pip install mutagen")
    try:
    ...
    tags.add(COMM(encoding=3, lang="eng", desc="Audio Encoding", text=f"MP3, {SAMPLE_RATE / 1000:.1f} kHz, {settings.bitrate} kbps"))
